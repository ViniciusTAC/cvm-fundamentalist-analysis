...
from models.tipo_relatorio_auditor import Tipo_relatorio_auditor

def process_csv_files(base_path):
    descricao_unica_set = set()
    tipo_relatorio_auditor_list = []
        ...
        for file in os.listdir(year_path):
            if file.endswith(".csv") and file.startswith("ipe_cia_aberta"):
                ...
                for _, row in df.iterrows():
                    descricao = row.get('TP_RELAT_AUD')
                    if pd.notnull(descricao) and descricao not in descricao_unica_set:
                        descricao_unica_set.add(descricao)
                        tipo_relatorio_auditor = Tipo_relatorio_auditor(_descricao=descricao)
                        tipo_relatorio_auditor_list.append(tipo_relatorio_auditor)

    return tipo_relatorio_auditor_list
