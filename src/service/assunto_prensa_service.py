...
from models.assunto_prensa import Assunto_prensa
        ...
        for file in os.listdir(year_path):
            if file.endswith(".csv") and file.startswith("ipe_cia_aberta"):
                ...
                for _, row in df.iterrows():
                    descricao = row.get("Assunto")
                    if pd.notnull(descricao) and descricao not in descricao_unica_set:
                        descricao_unica_set.add(descricao)
                        assunto_prensa = Assunto_prensa(_descricao=descricao)
                        assunto_prensa_list.append(assunto_prensa)

    return assunto_prensa_list
