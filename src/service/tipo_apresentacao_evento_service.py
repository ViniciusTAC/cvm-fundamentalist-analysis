...
from models.tipo_apresentacao_evento import Tipo_apresentacao_evento


def process_csv_files(base_path):
    descricao_unica_set = set()
    tipo_apresentacao_evento_list = []
        ...
        for file in os.listdir(year_path):
            if file.endswith(".csv") and file.startswith("ipe_cia_aberta"):
                ...
                for _, row in df.iterrows():
                    descricao = row.get("Tipo")
                    if pd.notnull(descricao) and descricao not in descricao_unica_set:
                        descricao_unica_set.add(descricao)
                        tipo_apresentacao_evento = Tipo_apresentacao_evento(
                            _descricao=descricao
                        )
                        tipo_apresentacao_evento_list.append(tipo_apresentacao_evento)

    return tipo_apresentacao_evento_list
