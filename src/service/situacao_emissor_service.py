...
from models.situacao_emissor import Situacao_emissor

def process_csv_files(base_path):
    descricao_unica_set = set()
    situacao_emissor_list = []
        ...
        for file in os.listdir(year_path):
            if file.endswith(".csv") and file.startswith("fca_cia_aberta_geral"):
                ...
                for _, row in df.iterrows():
                    descricao = row.get('Situacao_Emissor')
                    if pd.notnull(descricao) and descricao not in descricao_unica_set:
                        descricao_unica_set.add(descricao)
                        situacao_emissor = Situacao_emissor(_descricao=descricao)
                        situacao_emissor_list.append(situacao_emissor)

    return situacao_emissor_list
