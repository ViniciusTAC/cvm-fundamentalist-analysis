...
from models.setor_atividade import Setor_atividade

def process_csv_files(base_path):
    descricao_unica_set = set()
    setor_atividade_list = []
        ...
        for file in os.listdir(year_path):
            if file.endswith(".csv") and file.startswith("fca_cia_aberta_geral"):
                ...
                for _, row in df.iterrows():
                    descricao = row.get('Setor_Atividade')
                    if pd.notnull(descricao) and descricao not in descricao_unica_set:
                        descricao_unica_set.add(descricao)
                        setor_atividade = Setor_atividade(_descricao=descricao)
                        setor_atividade_list.append(setor_atividade)

    return setor_atividade_list
