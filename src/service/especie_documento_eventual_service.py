...
from models.especie_documento_eventual import Especie_documento_eventual


def process_csv_files(base_path):
    descricao_unica_set = set()
    especie_documento_eventual_list = []
        ...
        for file in os.listdir(year_path):
            if file.endswith(".csv") and file.startswith("ipe_cia_aberta"):
                ...
                for _, row in df.iterrows():
                    descricao = row.get("Especie")
                    if pd.notnull(descricao) and descricao not in descricao_unica_set:
                        descricao_unica_set.add(descricao)
                        especie_documento_eventual = Especie_documento_eventual(
                            _descricao=descricao
                        )
                        especie_documento_eventual_list.append(
                            especie_documento_eventual
                        )

    return especie_documento_eventual_list
