...
class ConexaoBanco:
    ...
    def inserir_ou_ignorar_assunto_prensa(self, assunto_prensa):
        try:
            cursor = self.connection.cursor()

            query = """
                INSERT INTO assunto_prensa (
                    descricao
                )
                VALUES (
                    ?
            ...
            values = (tratar_valor(assunto_prensa._descricao),)

            cursor.execute(query, values)
            # self.connection.commit()
            self.log_sucesso.info(
                f"Assunto: {assunto_prensa._descricao} inserida/atualizada com sucesso."
            )
            print(f"Assunto: {assunto_prensa._descricao} inserida com sucesso.")
        except sqlite3.Error as e:
            escrever_linha_em_branco(self.log_erro)
            escrever_linha_separador(self.log_erro)
            escrever_linha_em_branco(self.log_erro)
            self.log_erro.error(
                f"Erro ao inserir Assunto: {assunto_prensa._descricao}, erro: {e}."
            )
            escrever_linha_em_branco(self.log_erro)
            print(f"Erro ao inserir Assunto: {assunto_prensa._descricao}, erro: {e}.")
            # if commit:
