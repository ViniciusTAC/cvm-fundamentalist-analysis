...
class ConexaoBanco:
    ...
    def inserir_ou_ignorar_tipo_apresentacao_evento(self, tipo_apresentacao_evento):
        try:
            cursor = self.connection.cursor()

            query = """
                INSERT INTO tipo_apresentacao_evento (
                    descricao
                )
                VALUES (
                    ?
            ...
            values = (tratar_valor(tipo_apresentacao_evento._descricao),)

            cursor.execute(query, values)
            # self.connection.commit()
            self.log_sucesso.info(
                f"Tipo apresentacao evento: {tipo_apresentacao_evento._descricao} inserida/atualizada com sucesso."
            )
            print(
                f"Tipo apresentacao evento: {tipo_apresentacao_evento._descricao} inserida com sucesso."
            )
        except sqlite3.Error as e:
            ...
            escrever_linha_em_branco(self.log_erro)
            self.log_erro.error(
                f"Erro ao inserir Tipo apresentacao evento: {tipo_apresentacao_evento._descricao}, erro: {e}."
            )
            escrever_linha_em_branco(self.log_erro)
            print(
                f"Erro ao inserir Tipo apresentacao evento: {tipo_apresentacao_evento._descricao}, erro: {e}."
