...
class ConexaoBanco:
    ...
    def inserir_ou_ignorar_tipo_evento(self, tipo_evento):
        try:
            ...
            values = (
                tratar_valor(tipo_evento._descricao),
            )


            cursor.execute(query, values)
            # self.connection.commit()
            self.log_sucesso.info(f"Tipo evento: {tipo_evento._descricao} inserida/atualizada com sucesso.")
            print(f"Tipo evento: {tipo_evento._descricao} inserida com sucesso.")
        except sqlite3.Error as e:
            escrever_linha_em_branco(self.log_erro)
            escrever_linha_separador(self.log_erro)
            escrever_linha_em_branco(self.log_erro)
            self.log_erro.error(f"Erro ao inserir Tipo evento: {tipo_evento._descricao}, erro: {e}.")
            escrever_linha_em_branco(self.log_erro)
            print(f"Erro ao inserir Tipo evento: {tipo_evento._descricao}, erro: {e}.")
            # if commit:
