...
class ConexaoBanco:
    ...
    def inserir_ou_ignorar_escala_monetaria(self, escala_monetaria):
        try:
            cursor = self.connection.cursor()

            query = """
                INSERT INTO escala_monetaria (
                    descricao
                )
                VALUES (
                    ?
            ...
            values = (
                tratar_valor(escala_monetaria._descricao),
            )


            cursor.execute(query, values)
            # self.connection.commit()
            self.log_sucesso.info(f"Escala monetaria: {escala_monetaria._descricao} inserida/atualizada com sucesso.")
            print(f"Escala monetaria: {escala_monetaria._descricao} inserida com sucesso.")
        except sqlite3.Error as e:
            escrever_linha_em_branco(self.log_erro)
            escrever_linha_separador(self.log_erro)
            escrever_linha_em_branco(self.log_erro)
            self.log_erro.error(f"Erro ao inserir Escala monetaria: {escala_monetaria._descricao}, erro: {e}.")
            escrever_linha_em_branco(self.log_erro)
            print(f"Erro ao inserir Escala monetaria: {escala_monetaria._descricao}, erro: {e}.")
            # if commit:
