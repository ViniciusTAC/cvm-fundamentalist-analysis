...
class ConexaoBanco:
    ...
    def inserir_ou_ignorar_situacao_emissor(self, situacao_emissor):
        try:
            ...
            values = (
                tratar_valor(situacao_emissor._descricao),
            )


            cursor.execute(query, values)
            # self.connection.commit()
            self.log_sucesso.info(f"Situação emissor: {situacao_emissor._descricao} inserida/atualizada com sucesso.")
            print(f"Situação emissor: {situacao_emissor._descricao} inserida com sucesso.")
        except sqlite3.Error as e:
            escrever_linha_em_branco(self.log_erro)
            escrever_linha_separador(self.log_erro)
            escrever_linha_em_branco(self.log_erro)
            self.log_erro.error(f"Erro ao inserir Situação emissor: {situacao_emissor._descricao}, erro: {e}.")
            escrever_linha_em_branco(self.log_erro)
            print(f"Erro ao inserir Situação emissor: {situacao_emissor._descricao}, erro: {e}.")
            # if commit:
