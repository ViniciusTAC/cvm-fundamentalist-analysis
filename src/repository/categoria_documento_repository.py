...
class ConexaoBanco:
    ...
    def inserir_ou_ignorar_categoria_documento(self, categoria_documento):
        try:
            cursor = self.connection.cursor()

            query = """
                INSERT INTO categoria_documento (
                    descricao
                )
                VALUES (
                    ?
            ...
            values = (
                tratar_valor(categoria_documento._descricao),
            )


            cursor.execute(query, values)
            # self.connection.commit()
            self.log_sucesso.info(f"Categoria documento: {categoria_documento._descricao} inserida/atualizada com sucesso.")
            print(f"Categoria documento: {categoria_documento._descricao} inserida com sucesso.")
        except sqlite3.Error as e:
            escrever_linha_em_branco(self.log_erro)
            escrever_linha_separador(self.log_erro)
            escrever_linha_em_branco(self.log_erro)
            self.log_erro.error(f"Erro ao inserir Categoria documento: {categoria_documento._descricao}, erro: {e}.")
            escrever_linha_em_branco(self.log_erro)
            print(f"Erro ao inserir Categoria documento: {categoria_documento._descricao}, erro: {e}.")
            # if commit:
