...
class ConexaoBanco:
    ...
    def inserir_ou_ignorar_tipo_relatorio_auditor(self, tipo_relatorio_auditor):
        try:
            cursor = self.connection.cursor()

            query = """
                INSERT INTO tipo_relatorio_auditor (
                    descricao
                )
                VALUES (
                    ?
            ...
            values = (
                tratar_valor(tipo_relatorio_auditor._descricao),
            )


            cursor.execute(query, values)
            # self.connection.commit()
            self.log_sucesso.info(f"Tipo relatorio auditor: {tipo_relatorio_auditor._descricao} inserida/atualizada com sucesso.")
            print(f"Tipo relatorio auditor: {tipo_relatorio_auditor._descricao} inserida com sucesso.")
        except sqlite3.Error as e:
            escrever_linha_em_branco(self.log_erro)
            escrever_linha_separador(self.log_erro)
            escrever_linha_em_branco(self.log_erro)
            self.log_erro.error(f"Erro ao inserir Tipo relatorio auditor: {tipo_relatorio_auditor._descricao}, erro: {e}.")
            escrever_linha_em_branco(self.log_erro)
            print(f"Erro ao inserir Tipo relatorio auditor: {tipo_relatorio_auditor._descricao}, erro: {e}.")
            # if commit:
