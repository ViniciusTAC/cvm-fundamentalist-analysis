...
class ConexaoBanco:
    ...
    def inserir_ou_ignorar_moeda(self, moeda):
        try:
            ...
            values = (tratar_valor(moeda._descricao),)

            cursor.execute(query, values)
            # self.connection.commit()
            self.log_sucesso.info(
                f"Moeda: {moeda._descricao} inserida/atualizada com sucesso."
            )
            print(f"Moeda: {moeda._descricao} inserida com sucesso.")
        except sqlite3.Error as e:
            escrever_linha_em_branco(self.log_erro)
            escrever_linha_separador(self.log_erro)
            escrever_linha_em_branco(self.log_erro)
            self.log_erro.error(
                f"Erro ao inserir Moeda: {moeda._descricao}, erro: {e}."
            )
            escrever_linha_em_branco(self.log_erro)
            print(f"Erro ao inserir Moeda: {moeda._descricao}, erro: {e}.")
            # if commit:
