...
class ConexaoBanco:
    ...
    def inserir_ou_ignorar_ordem_exercicio(self, ordem_exercicio):
        try:
            cursor = self.connection.cursor()

            query = """
                INSERT INTO ordem_exercicio (
                    descricao
                )
                VALUES (
                    ?
            ...
            values = (
                tratar_valor(ordem_exercicio._descricao),
            )


            cursor.execute(query, values)
            # self.connection.commit()
            self.log_sucesso.info(f"Ordem exercicio: {ordem_exercicio._descricao} inserida/atualizada com sucesso.")
            print(f"Ordem exercicio: {ordem_exercicio._descricao} inserida com sucesso.")
        except sqlite3.Error as e:
            escrever_linha_em_branco(self.log_erro)
            escrever_linha_separador(self.log_erro)
            escrever_linha_em_branco(self.log_erro)
            self.log_erro.error(f"Erro ao inserir Ordem exercicio: {ordem_exercicio._descricao}, erro: {e}.")
            escrever_linha_em_branco(self.log_erro)
            print(f"Erro ao inserir Ordem exercicio: {ordem_exercicio._descricao}, erro: {e}.")
            # if commit:
