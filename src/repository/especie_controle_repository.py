...
class ConexaoBanco:
    ...
    def inserir_ou_ignorar_especie_controle(self, especie_controle):
        try:
            ...
            values = (
                tratar_valor(especie_controle._descricao),
            )


            cursor.execute(query, values)
            # self.connection.commit()
            self.log_sucesso.info(f"Especie controle: {especie_controle._descricao} inserida/atualizada com sucesso.")
            print(f"Especie controle: {especie_controle._descricao} inserida com sucesso.")
        except sqlite3.Error as e:
            escrever_linha_em_branco(self.log_erro)
            escrever_linha_separador(self.log_erro)
            escrever_linha_em_branco(self.log_erro)
            self.log_erro.error(f"Erro ao inserir Especie controle: {especie_controle._descricao}, erro: {e}.")
            escrever_linha_em_branco(self.log_erro)
            print(f"Erro ao inserir Especie controle: {especie_controle._descricao}, erro: {e}.")
            # if commit:
