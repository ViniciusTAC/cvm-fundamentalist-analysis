...
class ConexaoBanco:
    ...
    def inserir_ou_ignorar_especie_documento_eventual(self, especie_documento_eventual):
        try:
            cursor = self.connection.cursor()

            query = """
                INSERT INTO especie_documento_eventual (
                    descricao
                )
                VALUES (
                    ?
            ...
            values = (
                tratar_valor(especie_documento_eventual._descricao),
            )


            cursor.execute(query, values)
            # self.connection.commit()
            self.log_sucesso.info(f"Especie documento eventual: {especie_documento_eventual._descricao} inserida/atualizada com sucesso.")
            print(f"Especie documento eventual: {especie_documento_eventual._descricao} inserida com sucesso.")
        except sqlite3.Error as e:
            escrever_linha_em_branco(self.log_erro)
            escrever_linha_separador(self.log_erro)
            escrever_linha_em_branco(self.log_erro)
            self.log_erro.error(f"Erro ao inserir Especie documento eventual: {especie_documento_eventual._descricao}, erro: {e}.")
            escrever_linha_em_branco(self.log_erro)
            print(f"Erro ao inserir Especie documento eventual: {especie_documento_eventual._descricao}, erro: {e}.")
            # if commit:
