...
class ConexaoBanco:
    ...
    def inserir_ou_ignorar_setor_atividade(self, setor_atividade):
        try:
            ...
            values = (
                tratar_valor(setor_atividade._descricao),
            )


            cursor.execute(query, values)
            # self.connection.commit()
            self.log_sucesso.info(f"Setor atividade: {setor_atividade._descricao} inserida/atualizada com sucesso.")
            print(f"Setor atividade: {setor_atividade._descricao} inserida com sucesso.")
        except sqlite3.Error as e:
            escrever_linha_em_branco(self.log_erro)
            escrever_linha_separador(self.log_erro)
            escrever_linha_em_branco(self.log_erro)
            self.log_erro.error(f"Erro ao inserir Setor atividade: {setor_atividade._descricao}, erro: {e}.")
            escrever_linha_em_branco(self.log_erro)
            print(f"Erro ao inserir Setor atividade: {setor_atividade._descricao}, erro: {e}.")
            # if commit:
